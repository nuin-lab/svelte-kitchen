...
export function getAllImages(): PhotoData[] {
  return imageList.map((imageId) => ({ id: imageId, alt: 'Sample image from Lorem Picsum' }));
}

export function getImage(id: number): PhotoData | null {
  const imageId = imageList.at(id);

  return imageId ? { id: imageId, alt: 'Sample image from Lorem Picsum' } : null;
}
